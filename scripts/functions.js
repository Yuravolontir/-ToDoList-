...
export function createNewTodo(event) {
    ...
    if (!hebrewPattern.test(title)) {
        alert('כותרת המשימה חייבת להיות בעברית');
        return;
    }
    ...
    if (!hebrewPattern.test(description)) {
      alert('תיאור המשימה חייב להיות בעברית');
        return;
    }
    let dueDate = new Date(form.querySelector('#todo-date').value); // תאריך יעד
     ...
     if (dueDate.valueOf() > today.valueOf()+2592000000) // הגדרת תאריך יעד לא יכול להיות רחוק יותר מ-30 יום מהיום
     {
...
function renderTodos() {
 ...
 function showItemInList(list, item) {
    ...
    let deleteButton  = document.createElement('button'); // יצירת כפתור מחיקה
    deleteButton.innerText = 'מחק'; // הגדרת טקסט הכפתור
    deleteButton.addEventListener('click',deleteTodo); // הוספת מאזין לאירוע לחיצה
    ...
    li.appendChild(deleteButton); // הוספת אלמנט הכפתור למחיקה לאלמנט הרשימה
...
function deleteTodo(event) {
    ...
    event.preventDefault(); // למנוע רענון של הדף
    let id = event.target.parentElement.querySelector('input').id; // מזהה המשימה
    itemsList.forEach((item, index) => {
        if (item.id == id) {
            itemsList.splice(index, 1); // הסרת המשימה מהרשימה
        }
    })
    sessionStorage.setItem('itemsList', JSON.stringify(itemsList)); // שמירה בסשן סטורג׳
    localStorage.setItem('itemsList', JSON.stringify(itemsList)); // שמירה בלוקאל סטורג׳
    renderTodos(); // רינדור הרשימה המעודכנת
    alert('המשימה נמחקה בהצלחה'); // הודעה למשתמש
 ...
 function changeStatus(event) {
    ...
    itemsList.length = 0; // ריקון הרשימה הגלובלית
    ...
    export function clearCompletedTodos(event) {
    event.preventDefault(); // למנוע רענון של הדף
 ...
 sessionStorage.setItem('itemsList', JSON.stringify(itemsList)); // שמירה בסשן סטורג׳
    ...
    alert('כל המשימות הושלמו נמחקו בהצלחה'); // הודעה למשתמש
    

    completed.innerHTML = ''; // ריקון הרשימה של המשימות הושלמו
    renderTodos(); // רינדור הרשימה המעודכנת
...
export function SortByDate(event) {
    ...
    todoList.innerHTML = ''; // ריקון הרשימה
    completed.innerHTML = ''; // ריקון הרשימה
    const size = itemsList.length; // שמירת גודל הרשימה
    for (let i = 0; i < size - 1; i++) {
        for (let j = 0; j < size - i - 1; j++) {
            if (new Date(itemsList[j].dueDate) > new Date(itemsList[j + 1].dueDate)) {
                let temp = itemsList[j];
...
export function SortByPriority(event) {
    event.preventDefault(); // למנוע רענון של הדף
...
const size = itemsList.length; // שמירת גודל הרשימה
    for (let i = 0; i < size - 1; i++) {
        for (let j = 0; j < size - i - 1; j++) {
            let priorityValue;
            let priorityValue2;
            if (itemsList[j].priority === 'נמוכה')
               priorityValue = 3;
            if (itemsList[j].priority === 'בינונית')
                priorityValue = 2;
            if (itemsList[j].priority === 'גבוהה')
                 priorityValue = 1;


                if (itemsList[j+1].priority === 'נמוכה')
               priorityValue2 = 3;
            if (itemsList[j+1].priority === 'בינונית')
                priorityValue2 = 2;
            if (itemsList[j+1].priority === 'גבוהה')
                 priorityValue2 = 1;
...
export function loadFromLocalStorage() {
    ...
    if (items) {
         itemsList.length = 0;    //doubling defender
