...
export function SortByDate(event) {
    ...
    for (let i =0;i<size-1;i++) {
        for (let j = 0; j < size - i -1; j++) {
              ...
              let temp  = itemsList[j].dueDate; // שמירת המשימה הזמנית
              itemsList[j].Date = itemsList[j + 1].dueDate; // עדכון התאריך של המשימה הראשונה
            itemsList[j + 1].dueDate = temp; // עדכון התאריך של המשימה השנייה
            }
        }
    }// לולאת סידור הרשימה
    localStorage.setItem('itemsList', JSON.stringify(itemsList)); // שמירה בלוקאל סטורג׳
