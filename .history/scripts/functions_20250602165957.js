...
export function SortByDate(event) {
    ...
    for (let i =0;i<size-1;i++) {
        for (let j = 0; j < size - i -1; j++) {
            if (itemsList[j].Date > itemsList[j + 1].Date) { // השוואת תאריכים
              let temp  = itemsList[j].Date; // שמירת המשימה הזמנית
              itemsList[j].Date = itemsList[j + 1].Date; // עדכון התאריך של המשימה הראשונה
            itemsList[j + 1].Date = temp; // עדכון התאריך של המשימה השנייה
            }
        }
    }// לולאת סידור הרשימה
    localStorage.setItem('itemsList', JSON.stringify(itemsList)); // שמירה בלוקאל סטורג׳
